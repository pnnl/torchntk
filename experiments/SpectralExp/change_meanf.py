...
parser.add_argument('--lda', type=float, default=0.0)
...
for i in tqdm(range(len(n_list))):
    ...
    X,Y,X_test,Y_test = single_index(n,d,int(2*n/3),sigma=sigma,nonlin=sigmoid)
...
ax2.title.set_text('Relu activation, lr=%g,reg=%g'% (eta,lda))
ax2.legend()
...
fig.savefig('mf_weight_%g_%g_lr=%g_lda=%g.png'% (gamma1,gamma2,eta,lda))
...
ax1.title.set_text('Relu activation, lr=%g'% eta)
ax1.legend()
...
ax2.title.set_text('L2 regularization lda=%g'% lda)
ax2.legend()
...
fig.tight_layout(pad=1.5)
fig.savefig('mf_ck_ntk_%g_%g_lr=%g_lda=%g.png'% (gamma1,gamma2,eta,lda))

np.save('mf_NN_%g_%g_lr=%g_lda=%g.npy' % (gamma1,gamma2,eta,lda),NN_loss)
np.save('mf_spectra_%g_%g_lr=%g_lda=%g.npy' % (gamma1,gamma2,eta,lda),NN_weight_diff)
...
ax.set_ylabel('Training and Test Losses')
ax.title.set_text('Losses $d/n=$%g, $h/n=$%g,lr=%g,lda=%g'% (gamma1,gamma2,eta,lda))
ax.legend()

fig.tight_layout(pad=1.5)
fig.savefig('MF_Train_%g_%g_lr=%g_n=%d_lda=%g.png'% (gamma1,gamma2,eta,n_max,lda))
